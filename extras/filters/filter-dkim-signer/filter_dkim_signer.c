...
dkim_signer_on_msg_end(uint64_t id, size_t size)
{
	...
	if ((s = filter_api_transaction(id)) == NULL) {
		log_warnx("warn: on_msg_end: transaction failed");
		goto done;
	}
...
static void*
dkim_signer_tx_alloc(uint64_t id)
{
	struct dkim_signer *s = xmalloc(sizeof *s, "dkim_signer: tx_alloc");
...
static void
dkim_signer_tx_free(void *v)
{
	if (v == NULL)
		return;
	struct dkim_signer* s = v;
	while (!SIMPLEQ_EMPTY(&s->lines)) {
		struct entry *n = SIMPLEQ_FIRST(&s->lines);
...
main(int argc, char **argv)
{
	...
	filter_api_transaction_allocator(dkim_signer_tx_alloc);
	filter_api_transaction_destructor(dkim_signer_tx_free);
	if (c)
